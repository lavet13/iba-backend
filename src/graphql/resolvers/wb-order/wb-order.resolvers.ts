...
import { Prisma, WbOrder } from '@prisma/client';
import fs from 'fs';
  ...
  Query: {
    async wbOrders(_, args, ctx) {
      ...
      if (searchType.includes(SearchTypeWbOrders.Id)) {
        conditions.push({ id: { equals: Number.isFinite(+query) ? BigInt(query) : undefined } });
      }
    ...
    async searchWbOrders(_, args, ctx) {
      const query = args.input.query?.trim();
      if (query.length === 0) {
        return {
          edges: [],
      ...
      const wbOrders = await ctx.prisma.wbOrder.findMany({
        ...
        cursor,
        skip: cursor ? 1 : undefined, // Skip the cursor wbOrder for the next/previous page
        orderBy: [{ updatedAt: 'desc' }], // Order by id for consistent pagination
        where: {
